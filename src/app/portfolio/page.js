...
export default function PortfolioPage() {
  ...
  const { categories: projectCategories, fetchCategories, loading: categoriesLoading } = useProjectCategories();
  ...
  useEffect(() => {
    ...
    fetchData();
  }, []); // Removed dependencies to prevent multiple fetches

  // Filter categories to only include those with projects
  const categoriesWithProjects = projectCategories
    .filter(category => projects.some(project => project.category.name === category.name))
    .sort((a, b) => {
      const countA = projects.filter(project => project.category.name === a.name).length;
      const countB = projects.filter(project => project.category.name === b.name).length;
